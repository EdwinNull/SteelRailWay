...
from scripts.eval_from_ckpt import autocast, resolve_amp_dtype, safe_load_ckpt, strip_prefix
...
def main():
    ...
    torch.save({"mu": [t.cpu() for t in mu_ref], "var": [t.cpu() for t in var_ref]}, output_dir / "reference_stats.pt")
    ...
    baseline_scores_csv = output_dir / "baseline_scores.csv"
    ...
    for fold_idx, test_good in enumerate(fold_parts, start=1):
        ...
        peft_ckpt = output_dir / f"fold{fold_idx}_peft_cam{args.view_id}.pth"
        save_peft(peft_ckpt, peft, args, train_good=train_good, test_good=test_good)
        write_history_csv(output_dir / f"fold{fold_idx}_history.csv", history)
        ...
        scores_csv = output_dir / f"fold{fold_idx}_scores.csv"
    ...
    final_ckpt = output_dir / f"final_peft_cam{args.view_id}.pth"
    save_peft(final_ckpt, final_peft, args, train_good=good_ids)
    write_history_csv(output_dir / "final_history.csv", final_history)
    ...
    final_scores_csv = output_dir / "final_peft_scores.csv"
    ...
    with open(output_dir / "depth_peft_map.json", "w", encoding="utf-8") as f:
    ...
    print(f"PEFT map: {output_dir / 'depth_peft_map.json'}")
